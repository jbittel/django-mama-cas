...
sane defaults, but can customize behavior and improve security. Note that
...
.. attribute:: MAMA_CAS_ASYNC_CONCURRENCY
   ...
   limiting of requests. Setting this to a positive integer limits concurrent
   requests generated by an individual logout event. If the number of
   requests reaches this limit, additional requests block until there is room.
...
.. attribute:: MAMA_CAS_ATTRIBUTE_CALLBACKS
   ...
   name and attribute values. These values are merged together and returned
   along with a service or proxy validation success. Each callable is
   provided a single argument of the ``User`` for which validation is
